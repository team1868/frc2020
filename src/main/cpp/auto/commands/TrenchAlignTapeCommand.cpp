...
void TrenchAlignTapeCommand::Init() {
	...
	switch (currState_) {
		case (kPivotInit) :
			...
			if (fabs(desiredDeltaAngle_) > 2.0) {
				...
				//move back from trench slightly (0.2 feet)
				moveBackCommand_ = new DriveStraightCommand(navXSource_, talonSource_, anglePIDOutput_, distancePIDOutput_,
						robot_, 0.2);
				moveBackCommand_->Init();
				//start pivot command
				printf("ANGLE turning FOR PIVOT COMMAND: %f, abs angle turning to is %f (includes orig angle)\n", desiredDeltaAngle_, robot_->GetNavXYaw()+desiredDeltaAngle_);
...
bool TrenchAlignTapeCommand::IsDone() {
    return isDone_;
}

bool TrenchAlignTapeCommand::Abort() {
	return abort_;
}

void TrenchAlignTapeCommand::Reset() {
...
void TrenchAlignTapeCommand::ReadFromJetson() {
    /*
